...
async def get_trains(
    ...
    if schedule_date and schedule_date != datetime.now(pytz.timezone('Asia/Yekaterinburg')).date():
...
async def read_root(request: Request):
    ...
    await log_event(
        ...
        action="Просмотр главной страницы",
        request=request
    )
...
async def get_logs(
    ...
    action: Optional[str] = Query(None, description="Фильтр по действию (поиск подстроки)"),
...
async def get_logs_stats(request: Request):
    ...
    await log_event(
        event_type="api_request",
        action="Получение статистики логов",
        request=request
    )
    ...
    event_types = ['api_request', 'page_view', 'train_query', 'train_filter', 'date_select', 'error', 'train_detail_view']
    stats_by_event_type = {}
